...
static bool capslock_state; // caps lock
static bool scrlock_state;  // scroll lock
static bool numlock_state;  // number lock
static bool extcode_state;  // Extended Code Status
...
void keymap_handler(int idt_index) {
    ...
    // Tell the interrupt control chip that the interrupt has been processed (it seems that there is nothing wrong with not doing this)
    send_eoi(idt_index);
    ...
    if (extcode_state)
    {
        ...
        // Modify the scan code, add 0xe0 prefix
        scancode |= 0xe000;
    ...
    // Whether it is a broken code, the button is lifted
    bool breakcode = ((scancode & 0x0080) != 0);
    if (breakcode)
    {
        // if yes then set state
        keymap[makecode][ext] = false;
        return;
    }

    // The following is the pass code, press the button
    keymap[makecode][ext] = true;

    // Do you need to set the LED light
    bool led = false;
